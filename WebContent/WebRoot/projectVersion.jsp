...
<script type="text/javascript">
  function tableAddRow()
  {
   ...
   var col;  //列对象
    ...
    col.align="center";
    col.innerHTML = "<input class='form-control' id='versionCode0' name='versionCode0' type='text' width='200'/>";
...
<%
	//获得初始化页面的所有参数
	UpdateProjectVersionResponse resp = (UpdateProjectVersionResponse)request.getAttribute("response");
...
<!-- navigate bar 导航 -->
  ...
  <div class="container" style="width:50%;">
    ...
    <div class="row">
	  ...
	  <form accept-charset="UTF-8" action="<%=basePath%>saveProjectVersion" class="new_post" data-remote="true" id="new_post" method="post"><div style="display:none"></div>
	  		<div class="field">
	  ...
	  <table id="versionTable" class="table1" border="1">
		<tr>
		<td width="200" align="center"><h4><strong>版本号<br> </strong></h4></td>
		...
		<%
			if(null != pvList && pvList.size()>0){
				for(ProjectVersion pv : pvList){
					String vc = pv.getVersion_code();
					Integer vid = pv.getId();
					String mtime = TimeUtil.dateFormat(pv.getMtime());
					
			    	
			    	//动态添加行
			    	%>
			    	<tr>
			    	<td align="center"><h5><input class="form-control" id="versionCode<%=vid %>" name="versionCode<%=vid %>" type="text" width="200" value="<%=vc %>"></h5></td>
			    	<td align="center"><h5><%=mtime %></h5></td>
			    	<td align="center"><h5><a href="<%=basePath%>deleteProjectVersion?vid=<%=vid %>&pid=<%=pm.getId() %>">删除</a></h5></td>
			    	</tr>
			    	<%
				}
			}
		...
		<input class="btn btn-success" name="new" type="button" onclick="tableAddRow();" value="新增版本"> </input>
  		&nbsp;&nbsp;
  		<input class="btn btn-primary" name="commit" type="submit" value="保存版本"> </input>
  		</div>
  		</form>
